...
def parse_artists(results: List[Dict[str, Any]], uploaded: bool = False) -> List[Dict[str, Any]]:
    artists = []
...
def parse_library_albums(response: Dict[str, Any], request_func: Callable, limit: Optional[int]) -> List[Dict[str, Any]]:
...
def parse_albums(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    albums = []
    for result in results:
        data = result[MTRIR]
        album = {}
...
def parse_library_podcasts(response: Dict[str, Any], request_func: Callable, limit: Optional[int]) -> List[Dict[str, Any]]:
    results = get_library_contents(response, GRID)
    if results is None:
        return []
...
def parse_library_artists(response: Dict[str, Any], request_func: Callable, limit: Optional[int]) -> List[Dict[str, Any]]:
...
def pop_songs_random_mix(results: Optional[Dict[str, Any]]) -> None:
...
def parse_library_songs(response: Dict[str, Any]) -> Dict[str, Any]:
    results = get_library_contents(response, MUSIC_SHELF)
    pop_songs_random_mix(results)
    return {"results": results, "parsed": parse_playlist_items(results["contents"]) if results else results}


def get_library_contents(response: Dict[str, Any], renderer: List[str]) -> Optional[Dict[str, Any]]:
    """
